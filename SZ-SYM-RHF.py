...
expr=0.56*sympy.exp(1*R*R)
[expr.series(R,1.21233).removeO(),
expr.subs(R,1.46)]
...
#
    ...
    if  type(ARG)==float and ARG < 1.0e-6:
        return 1 -ARG/3.
    if  type(ARG)==sympy.numbers.Zero and ARG < 1.0e-6:
        return 1 -ARG/3.
    else:
        #print("F0:ARG",ARG)
...
stringQ+='ring r1=0,(x,y,e,R),dp;\n'+'poly OBJ='+str(OF)+';\n'
stringQ+='list diffs;\n'
stringQ+='for(int i=1;i<=nvars(r1); i=i+1){diffs=insert(diffs,diff(OBJ,var(i)));}\n'
stringQ+='poly fR=100*R-146;\n'
stringQ+='ideal I=fR;\n'
stringQ+='for(int i=1;i<=nvars(r1)-1; i=i+1){I=I+diff(OBJ,var(i));}\n'
stringQ+='ideal SI=std(I);\n'
stringQ+='ring s=0,(x,y,e,R),lp;\n'
stringQ+='setring s;\n'
stringQ+='ideal j=fglm(r1,SI);\n'
...
ee,vv=np.linalg.eig(mx)
ww=vv.transpose()
for eee,www in zip(ee,ww):
    if np.abs(np.imag(eee)) < 1.0e-10:
        #print(eee)
        print([np.real(np.dot(www.conj(),np.dot(mop,www))) for mop in [mx,my,me]])
...
[[1.00000142599786, 0.450770411647788], [0.450770411647788, 1.00000142599786]]
[[2.16431256194901, 0.167012865864049], [0.167012865864049, 0.760032943565085]]
KAB
KAB
...
for x in [V11A,V12A,V22A,V11B,V12B,V22B]:
    print(x.subs(R,1.46))
